...
////// CALCULATOR //////
////////////////////////
//below, we're writing 4 functions that will perform the operation of their name
// CODE HERE
const add = (num1, num2) => num1 + num2
const subtract = (num1, num2) => num1 - num2
const multiply = (num1, num2) => num1 * num2
const divide = (num1, num2) => num1 / num2 //next we have to write a function to TAKE IN the above functions as arguments, this is called a Higher Order Function

const calculator = (num1, num2, callback) => {
    if(+num1 && +num2){ // _+_ shorthand for a number, essentially setting a variable equal to a number if possible
        num1 = +num1
        num2 = +num2
        return callback(num1, num2)
    } else {
        console.log(`math needs nummies`)
    }
}
...
// speciesDiscount(catProducts, flatDiscount, 5) //
console.log(catProducts)

const categoryDiscount = (arr, category, callback, discount) => { //what's new here? the category
    arr.forEach(product => {
...
categoryDiscount(catProducts, 1, percentDiscount, .1)
console.log(catProducts)
...
inventoryDiscount(dogProducts, 50, flatDiscount, 2) //this is called invoking a function
...
const mappedColors = colors.map(colors => `pink`)
...
const formalGreeting = names => {
    const greeting = names.map(name => `${name}`)
    return greeting
}
...
const identifier = (arr) => {
    let person = arr.filter(person => person.programmer) //when running filter method, everything on the right side of _=_ will evaluate to 'true'
    return person[0]
}
...
const remaing = expenses.reduce((acc, cv) => {return acc - cv.amount}, 2000) //cv is an object, amount represents a 'key' 
console.log(remaing)